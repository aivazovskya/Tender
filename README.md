...
9. **Оценка конкуренции и вероятности победы (Module 9)**:
   ...
   - **Research Spike в GraphQL Goszakup**: В текущей версии API `goszakup.gov.kz` протокол итогов с именами участников не передаётся в объекте `TrdBuy`, поэтому Phase 1 опирается на собственные анонимизированные данные платформы.
