...
## 📜 10. Правовой стандарт доступа к данным (Compliance) и План Б

### 10.1 Официальный доступ к данным (goszakup.gov.kz / portal.sk.kz)
1. **Официальный API-канал**: При работе системы в коммерческой эксплуатации авторизованные запросы к веб-сервисам ЕГСЗ РК (`ows.goszakup.gov.kz`) и АО "Самрук-Казына" выполняются с использованием действующего персонального токена участника закупки (`GOSZAKUP_API_TOKEN` / `SAMRUK_API_TOKEN`).
2. **Compliance-стандарт при отсутствии REST API**:
   - Полное соблюдение `robots.txt` источников и запрет на обход анти-бот систем или капчи.
   - Ограничение скорости запросов (`RateLimiter` token-bucket: максимум 1 запрос в секунду).
   - Прозрачный заголовок идентификации бота: `User-Agent: TenderAI-IngestBot/1.0 (+https://tenderai.kz)`.

### 10.2 Операционный План Б при потере доступа к источнику
В случае изменения вёрстки порталов или блокировки IP-адресов автоматическая служба **IngestionHealthService** выполняет следующий алгоритм:

1. **Автоматическое логирование и Telegram-алерт**: При 3 подряд ошибках парсинга или молчании источника &gt;6 часов в Telegram администратора отправляется уведомление со статусом 🔴 `FAILED` или 🟡 `WARNING`.
2. **Аварийный фоллбэк на ручной импорт**: Администратор может загрузить реестр новых закупок через JSON/CSV экспорт порталов во вкладке **Администрирование &rarr; Ручной импорт**.
3. **Регламент уведомления клиента (SLA < 2ч)**: В течение 2 часов клиент информируется о переходе на альтернативный канал получения данных.
